...
void Hal_timer_init(void)
{
    ...
    Timer->timerxcontrol.bits.TimerMode = 0;        // pre_running
    Timer->timerxcontrol.bits.OneShot = 0;
    Timer->timerxcontrol.bits.TimerSize = 0;        // 16bit counter mod
    Timer->timerxcontrol.bits.TimerPre = 0;         // set divider 1 
    Timer->timerxcontrol.bits.IntEnable = 1;        // interrupt on 
    Timer->timerxload = 0;                          // road register on
    Timer->timerxvalue = 0xFFFFFFFF;                // set count register 0xFFFFFFFF
